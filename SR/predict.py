...
def save_re2img(sr_pred):
    ...
    output_rgb_dataset = driver.Create(save_tif_path, sr_pred.shape[0], sr_pred.shape[1], data_ch, gdal.GDT_Byte)
    #output_rgb_dataset.SetGeoTransform(geotransform)
    #output_rgb_dataset.SetProjection(projection)

    for i in range(band_num):
        output_band = output_rgb_dataset.GetRasterBand(i + 1)
        output_band.WriteArray(sr_pred[:, :, i])

    

def test(args):

    train_path    = './data/train_5ch/'
    eval_path     = './data/val_5ch/'
    result_path   = './results/'
    test_data_dir = './data/val_5ch/'
    ...
    model_name = './checkpoints/train/epoch_40.pth'
    print(model_name)
