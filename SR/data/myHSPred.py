...
class myHSPredData(data.Dataset):
    ...
    def __init__(self, image_dir, augment=None, use_3D=False):
        ...
        for i in self.image_folders_LR:
    ...
    def __getitem__(self, index):
        ...
        ms, lms = utils.data_augmentation(ms, mode=aug_num), utils.data_augmentation(lms, mode=aug_num)

        ms = torch.from_numpy(ms.copy()).permute(2, 0, 1)
        lms = torch.from_numpy(lms.copy()).permute(2, 0, 1)
        #gt = torch.from_numpy(gt.copy()).permute(2, 0, 1)

        return ms, lms, load_dir_LR

    def __len__(self):
        return len(self.image_files_LR)*self.factor
