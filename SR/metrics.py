...
def compare_mpsnr(x_true, x_pred, data_range):
	...
	# Calculate PSNR for each channel and print the results
	total_psnr = []
	...
	for k in range(x_true.shape[2]):
		...
		psnr_value = compare_psnr(x_true[:, :, k], x_pred[:, :, k], data_range=data_range)
		print(f"PSNR for channel {k}: {psnr_value}")
